	...
	IGameEventHandler<PlayerConnectedEvent>
{
	...
	private float GetScore(PlayerState PlayerState)
	{
		return AutoBalanceMode switch
		{
			AutoBalanceMode.Random => Random.Shared.NextSingle(),
			AutoBalanceMode.LowestScorers => PlayerState.Components.Get<PlayerScore>() is { } playerScore ? playerScore.Score : 0,
			AutoBalanceMode.LowestScorersLastRound => PlayerState.Components.Get<PlayerScore>() is { } playerScore ? playerScore.ScoreHistory.LastOrDefault() : 0,
			_ => 0
		};
	}

	private void AssignTeam(PlayerState PlayerState)
	{
		var AssignedTeam = GetRandomTeam();
		PlayerState.AssignTeam(AssignedTeam);
	}
	...
	{
		AssignTeam(eventArgs.PlayerState);
	}
}
